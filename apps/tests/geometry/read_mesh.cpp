...
TEST_CASE("read_msh", "[geometry]")
{
    ...
    REQUIRE_THROWS_AS(io.read_msh(std::format("{}NOMESH.msh", AssetDir::tetmesh_path())),
                      GeometryIOError);
}
...
{
    SimplicialComplexIO io;
    auto mesh = io.read_obj(std::format("{}cube.obj", AssetDir::trimesh_path()));
    REQUIRE(mesh.vertices().size() == 8);
    REQUIRE(mesh.edges().size() == 0);
    ...
    REQUIRE_THROWS_AS(io.read_obj(std::format("{}NOMESH.obj", AssetDir::trimesh_path())),
                      GeometryIOError);
}
