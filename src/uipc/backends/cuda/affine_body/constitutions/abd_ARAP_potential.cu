#include <affine_body/constitutions/abd_ARAP_potential.h>
...
{
REGISTER_SIM_SYSTEM(ABDARAPPotential);

void ABDARAPPotential::do_build(AffineBodyConstitution::BuildInfo& info)
{
    auto scene = world().scene();
    // Check if we have the ABDARAPPotential constitution
    auto uids = scene.constitution_tabular().uids();
    if(!std::binary_search(uids.begin(), uids.end(), ABDARAPPotential::ConstitutionUID))
    {
        throw SimEngineException(fmt::format("ABDARAPPotential requires ABDARAPPotential Constitution (UID={})",
                                             ABDARAPPotential::ConstitutionUID));
    }
}

U64 ABDARAPPotential::get_constitution_uid() const
{
    return ConstitutionUID;
}

void ABDARAPPotential::Impl::filter(const AffineBodyDynamics::FilteredInfo& info,
                                    WorldVisitor& world)
{
...
void ABDARAPPotential::Impl::compute_energy(const AffineBodyDynamics::ComputeEnergyInfo& info)
{
...
void ABDARAPPotential::Impl::compute_gradient_hessian(const AffineBodyDynamics::ComputeGradientHessianInfo& info)
{
    using namespace muda;
    auto N = info.qs().size();
...
void ABDARAPPotential::Impl::_build_on_device()
{
...
void ABDARAPPotential::do_filter(AffineBodyDynamics::FilteredInfo& info)
{
    m_impl.filter(info, world());
}

void ABDARAPPotential::do_compute_energy(AffineBodyDynamics::ComputeEnergyInfo& info)
{
    m_impl.compute_energy(info);
}

void ABDARAPPotential::do_compute_gradient_hessian(AffineBodyDynamics::ComputeGradientHessianInfo& info)
{
